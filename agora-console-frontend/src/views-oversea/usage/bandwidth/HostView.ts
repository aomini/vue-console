  ...
  template: `
    <div v-loading="loading">
      ...
      >
        ...
        <el-table-column
          prop="hostMax"
          :label='this.$t("hostPeak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.hostMax | formatUsage('bandwidth') }}
          </template>
        </el-table-column>
      </el-table>
...
export default class HostView extends Vue {
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => {
      if (this.peakValue < x.hostMax) {
        this.peakValue = x.hostMax
      }
      return [{ date: moment(x.date).format('MM-DD'), usage: x.hostMax, type: this.$t('totalPeak'), format: 'channel' }]
    })
      ...
      if (this.condition.projectId === '0' && this.settings.includes('Bandwidth Aggregate')) {
        ...
        for (const data of usageData) {
          if (data.date in usageObj) {
            usageObj[data.date] = usageObj[data.date] + data.hostMax
          } else {
            usageObj[data.date] = data.hostMax
          }
        }
        const dataArray = []
        for (const date of Object.keys(usageObj)) {
          dataArray.push({ date, hostMax: usageObj[date] })
        }
        this.data = dataArray
      } else {
