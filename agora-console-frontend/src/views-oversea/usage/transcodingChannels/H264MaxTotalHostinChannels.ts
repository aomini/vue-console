  ...
  template: `
    <div v-loading="loading">
      ...
      <line-chart
        v-if="!isEmpty"
        :data="lineData"
        :peakText='$t("H264 Max Total Host-in Concurrent channels")'
      ...
      >
        ...
        <el-table-column
          prop="avcTotalHostin"
          :label='this.$t("avcTotalHostIn")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.avcTotalHostin | formatUsage('channels') }}
          </template>
        </el-table-column>
      </el-table>
...
export default class H264MaxTotalHostinChannels extends Vue {
  loading = false
  isEmpty = false
  peakValue = 0
  data: any = []
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => {
      if (x.avcTotalHostin > this.peakValue) {
        this.peakValue = x.avcTotalHostin
      }
      return [
        {
          date: moment(x.date).format('MM-DD'),
          usage: x.avcTotalHostin,
          type: this.$t('avcTotalHostIn'),
          format: 'channel',
        },
      ]
    })
      ...
      if (this.condition.projectId === '0' && this.settings.includes('Channels Aggregate')) {
        ...
        for (const data of usageData) {
          if (data.date in usageObj) {
            usageObj[data.date] = usageObj[data.date] + data.avcTotalHostin
          } else {
            usageObj[data.date] = data.avcTotalHostin
          }
        }
        const dataArray = []
        for (const date of Object.keys(usageObj)) {
          dataArray.push({ date, avcTotalHostin: usageObj[date] })
        }
        this.data = dataArray
      } else {
