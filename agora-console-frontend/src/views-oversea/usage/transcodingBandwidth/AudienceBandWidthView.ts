  ...
  template: `
    <div v-loading="loading">
      ...
      >
        ...
        <el-table-column
          prop="transcodingBotMax"
          :label='this.$t("botPeak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.transcodingBotMax | formatUsage('bandwidth') }}
          </template>
        </el-table-column>
      </el-table>
...
export default class AudienceBandWidthView extends Vue {
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => {
      if (this.peakValue < x.transcodingBotMax) {
        this.peakValue = x.transcodingBotMax
      }
      return [
        {
          date: moment(x.date).format('MM-DD'),
          usage: x.transcodingBotMax,
  ...
  async getUsageInfo() {
    this.loading = true
    try {
      if (this.condition.projectId === '0' && this.settings.includes('Transcoding Bandwidth')) {
        ...
        for (const data of usageData) {
          if (data.date in usageObj) {
            usageObj[data.date] = usageObj[data.date] + data.transcodingBotMax
          } else {
            usageObj[data.date] = data.transcodingBotMax
          }
        }
        const dataArray = []
        for (const date of Object.keys(usageObj)) {
          dataArray.push({ date, transcodingBotMax: usageObj[date] })
        }
        this.data = dataArray
      } else {
  ...
  init() {
    ...
    if (projectSetting) this.settings = projectSetting['Transcoding Bandwidth'] || []
  }
