...
import '../Usage.less'
import { BusinessEnum, MeterModelEnum } from '@/views-oversea/usage/Constant'
...
export default class CloudProxyView extends Vue {
  ...
  async setType(type: string) {
    ...
    this.$router.push({ name: 'usage.cloudProxy.duration.' + type, query: Object.assign({}, this.condition) } as any)
  }
  ...
  setDate(intervalType: number) {
    ...
    if (this.condition.type) {
      this.$router.push({
        name: 'usage.cloudProxy.duration.' + this.condition.type,
