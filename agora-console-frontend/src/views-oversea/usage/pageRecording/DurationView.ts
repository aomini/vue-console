    ...
    <div v-loading="loading" class="cloud-container">
      <div>
        ...
        <div v-if="!isEmpty" class="card mt-20">
          ...
          <div>
            {{ $t('PageRecordingBillingExplain') }}
            <a :href="$t('Page recording billing url')" target="_blank">{{ $t('billing FAQ') }}</a>
          </div>
...
export default class DurationView extends Vue {
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => [
      { date: moment(x.date).format(displayFormat), usage: x.audio + x.videoSd + x.videoHd, type: this.$t('hd') },
      {
  ...
  getBarChartInfo() {
    const { hdSum, hdpSum } = this.data.reduce(
      (pre: any, cur: any) => {
        return {
          hdSum: pre.hdSum + cur.audio + cur.videoSd + cur.videoHd,
          hdpSum: pre.hdpSum + cur.videoHdp,
        }
      },
      { hdSum: 0, hdpSum: 0 }
    )

    const total = Math.max(hdSum + hdpSum, 1)
    const dataArray = [
      {
        index: '0',
        type: this.$t('hd'),
  ...
  async getUsageInfo() {
    ...
    try {
      ...
      this.tableData = this.data.map((x: any) => {
        const total = x.audio + x.videoSd + x.videoHd + x.videoHdp
        return {
          date: x.date,
          hd: x.audio + x.videoSd + x.videoHd,
          hdp: x.videoHdp,
          total: total,
        }
      })
