...
export default class HostView extends Vue {
  loading = false
  isEmpty = false
  data: any = []
  tableData: any = []
  ...
  getLineChartInfo() {
    ...
    if (this.settings.includes('Separate SD')) {
      flatData = this.data.map((x: any) => [
        { date: moment(x.date).format(displayFormat), usage: x.audioHost, type: this.$t('audio') },
        { date: moment(x.date).format(displayFormat), usage: x.sdHost, type: this.$t('sd') },
        { date: moment(x.date).format(displayFormat), usage: x.hdHost, type: this.$t('hd') },
        {
          date: moment(x.date).format(displayFormat),
          usage: x.video1080pHost,
          type: this.$t('Video Total Duration(Full HD)'),
        },
        { date: moment(x.date).format(displayFormat), usage: x.video2kHost, type: this.$t('Video2K') },
        { date: moment(x.date).format(displayFormat), usage: x.video4kHost, type: this.$t('Video2K+') },
      ])
    } else {
      flatData = this.data.map((x: any) => [
        { date: moment(x.date).format(displayFormat), usage: x.audioHost, type: this.$t('audio') },
        { date: moment(x.date).format(displayFormat), usage: x.sdHost + x.hdHost, type: this.$t('hd') },
        {
          date: moment(x.date).format(displayFormat),
          usage: x.video1080pHost,
          type: this.$t('Video Total Duration(Full HD)'),
  ...
  getBarChartInfo() {
    ...
    const { audioSum, sdSum, hdSum, hdpSum, video2kSum, video4kSum } = this.data.reduce(
      (pre: any, cur: any) => {
        return {
          audioSum: pre.audioSum + cur.audioHost,
          sdSum: pre.sdSum + cur.sdHost,
          hdSum: pre.hdSum + cur.hdHost,
          hdpSum: pre.hdpSum + cur.video1080pHost,
          video2kSum: pre.video2kSum + cur.video2kHost,
          video4kSum: pre.video4kSum + cur.video4kHost,
        }
      },
  ...
  async getUsageInfo() {
    ...
    try {
      ...
      this.tableData = this.data.map((x: any) => {
        const total = x.audioHost + x.sdHost + x.hdHost + x.video1080pHost + x.video2kHost + x.video4kHost
        if (this.settings.includes('Separate SD')) {
          return {
            date: x.date,
            audio: x.audioHost,
            sd: x.sdHost,
            hd: x.hdHost,
            hdp: x.video1080pHost,
            video2k: x.video2kHost,
            video4k: x.video4kHost,
            total: total,
          }
        } else {
          return {
