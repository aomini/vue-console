...
@Component({
  template: `
    <div class="uap-setting">
      <div class="module-title">{{ $t('CloudUsageTitle') }}</div>
      <div class="card">
        <h3 class="heading-dark-03 mb-20">{{ $t('MaxChannel') }}</h3>
        <div>{{ $t('Hint1') }} {{ maxSubscribeLoad }}</div>
        <div v-if='region==="1"'>{{ $t('Hint2') }} 100</div>
        <div v-else>{{ $t('Hint2') }} 50</div>
        <div>{{ $t('Hint3') }}</div>
...
export default class CloundRecordingSettingView extends Vue {
  loading = false
  applyLoading = false
  vids = ''
    ...
    if (this.$route.query.projectId && this.$route.query.projectId !== '0') {
      try {
        ...
        const ret = await this.$http.get(`/api/v2/usage/uap/setting`, {
          params: { vids: this.condition.vids, cloudTypeId: CloudTypeMap['CloudRecording'] },
        })
        this.loading = false
        if (ret.data) {
  ...
  async updateSetting() {
    const defaultMax = this.region === '1' ? 100 : 50
