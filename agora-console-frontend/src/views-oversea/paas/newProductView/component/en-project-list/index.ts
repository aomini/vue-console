...
import SecretKeyDialog from '@/views-oversea/paas/component/secret-key-dialog'
...
@Component({
  ...
  template: ` <div class="mini-project-list">
    <div class="list-title cursor-pointer" v-if="showTitle" @click="setShowStep()"><span>< </span>{{ $t('Projects') }}</div>
    <div class="projects">
      <div class="project-item" v-for="item in projectList" :key="item.id">
        ...
        <el-tooltip effect="light" placement="top" content="View Credentials">
          <i v-if="item.AppInfo && item.AppInfo.disabled === 0" class="iconfont iconpaas-icon-key-blue" w:mr="12px" w:text="gray-100 hover:(green-350)" style="font-size: 20px;" @click="showTwoFactorVerificationVisible(item)"></i>
        </el-tooltip>
        <el-switch
