...
@Component({
  ...
  template: `
    <div class="upload avatar-component">
      ...
      >
        <i class="el-icon-plus avatar-uploader-icon" v-show="!url&&showIcon"></i>
        <span class="link heading-grey-13" v-show="!url&&!showIcon">{{ $t('Upload') }}</span>
...
export default class AvatarUpload extends Vue {
  ...
  updateUrl(url: string, id: any, ossKey: string) {
    this.$emit('updateURL', url, id, ossKey)
  }
  showUrl(url: string) {
    this.url = url
  }
