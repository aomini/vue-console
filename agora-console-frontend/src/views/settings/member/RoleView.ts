...
@Component({
  ...
  template: `
    <div class="page-v3 role-page">
      ...
      <div w:m="b-10px t-20px" w:flex="~" w:align="center" w:justify="between">
        <console-button class="console-btn-primary" :disabled="addingState" @click="addRole">
          {{ $t('AddRole') }}
        </console-button>
        <el-checkbox v-model="showVendorCreator" @change="changeShowVendorCreator" v-if="!isMember">
          {{ $t('ShowVendorCreator') }}
        </el-checkbox>
      </div>
      <div class="card">
...
export default class RoleView extends Vue {
  ...
  async changeShowVendorCreator(status: boolean) {
    this.$confirm(
      status ? (this.$t('OpenVendorCreatorHint') as string) : (this.$t('CloseVendorCreatorHint') as string),
      this.$t('VendorCreatorConfirmTitle') as string,
      {
      ...
      .then(async () => {
        this.loading = true
        await companyExtraSetting.setCompanyField(CompanyFieldType.showVendorCreator, this.showVendorCreator ? 1 : 0)
        this.$message.success(this.$t('Updated successfully') as string)
        this.loading = false
      })
      .catch(async () => {
        this.showVendorCreator = !this.showVendorCreator
      })
  }
}
