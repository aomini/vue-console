    ...
    <div>
      <div class="usage-title-box-with-remaining d-flex justify-between">
        <div class="d-flex align-center">
          ...
          <div
            class="tab-button"
            v-if='settings.includes("Standard Bandwidth Audience")'
            :class='{ "active-btn": condition.type === "Standard Bandwidth Audience", "switch-btn": true }'
            @click='setType("Standard Bandwidth Audience")'
          >
            {{ $t('audience') }}
          </div>
...
export default class BandWidthView extends Vue {
  settings: any = ['Standard Bandwidth Audience']
  condition: any = {
    type: 'Standard Bandwidth Audience',
    intervalType: 0,
    fromTs: undefined,
    endTs: undefined,
    model: 'bandwidth',
  ...
  async setType(type: string) {
    ...
    this.$router.push({
      name: 'usage.standard.bandwidth.' + this.condition.type,
  ...
  init() {
    ...
    if (projectSetting) {
      this.settings = projectSetting['Standard Bandwidth'] || []
    }
