  ...
  template: `
    <div v-loading="loading">
      <div class="usage-title-box cloud-title d-flex justify-between align-center">
        <div class="d-inline-block mr-3 page-title">{{ $t('FusionCDNBandwidthTitle') }}</div>
      ...
      <bar-chart v-if="!isEmpty" :data="barData" type="cdnBandwidth" class="mb-60"> </bar-chart>
      <line-chart
        v-if="!isEmpty"
        :data="lineData"
      ...
      >
        ...
        <el-table-column
          prop="total"
          :label='this.$t("All Bandwidth Peak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.total | kbpsToMbps }}
          </template>
        </el-table-column>
        <el-table-column
          prop="cn"
          :label='this.$t("China Bandwidth Peak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.cn | kbpsToMbps }}
          </template>
        </el-table-column>
        <el-table-column
          prop="abroad"
          :label='this.$t("Ex-China Bandwidth Peak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.abroad | kbpsToMbps }}
          </template>
        </el-table-column>
      </el-table>
      <div v-if="!isEmpty" class="card mt-20">
        ...
        <div>
          {{ $t('FusionCDNHint') }}
          <a :href="$t('FusionCDNUrl')" target="_blank">{{ $t('billing FAQ') }}</a>
        </div>
...
export default class CDNBandwidthView extends Vue {
  condition: any = {
    intervalType: 0,
    fromTs: undefined,
    endTs: undefined,
  ...
  async getUsageInfo() {
    ...
    try {
      ...
      this.data.forEach((item: any) => {
        if (!item.areaCdnUsageMap.cn) {
          item.areaCdnUsageMap.cn = {
            totalBandwidth: 0,
            monthBandwidth95: 0,
            totalTraffic: 0,
          }
        }
        if (!item.areaCdnUsageMap.abroad) {
          item.areaCdnUsageMap.abroad = {
            totalBandwidth: 0,
            monthBandwidth95: 0,
            totalTraffic: 0,
          }
        }
        if (!item.areaCdnUsageMap.all) {
          item.areaCdnUsageMap.all = {
            totalBandwidth: 0,
            monthBandwidth95: 0,
            totalTraffic: 0,
          }
      ...
      this.tableData = this.data.map((x: any) => {
        const total = x.areaCdnUsageMap.all.totalBandwidth
        const cn = x.areaCdnUsageMap.cn.totalBandwidth
        const abroad = x.areaCdnUsageMap.abroad.totalBandwidth
        return Object.assign({}, { date: x.date, total: total, cn: cn, abroad: abroad })
      })
    ...
    flatData = this.data.map((x: any) => [
      {
        ...
        usage: Math.round(x.areaCdnUsageMap.all.totalBandwidth / 1000),
        type: this.$t('All Bandwidth Peak'),
        format: 'channel',
      },
      {
        date: moment(x.date).format(displayFormat),
        usage: Math.round(x.areaCdnUsageMap.cn.totalBandwidth / 1000),
        type: this.$t('China Bandwidth Peak'),
        format: 'channel',
      },
      {
        date: moment(x.date).format(displayFormat),
        usage: Math.round(x.areaCdnUsageMap.abroad.totalBandwidth / 1000),
        type: this.$t('Ex-China Bandwidth Peak'),
        format: 'channel',
      },
    ])

    const dataArray = [].concat(...flatData)
    this.lineData = dataArray.reduce((result: any, obj: any) => {
      const item = result.find((o: any) => o.date === obj.date && o.type === obj.type)
      item ? (item.usage += obj.usage) : result.push(obj)
      return result
    }, [])
  }
  ...
  getBarChartInfo() {
    let dataArray = []
    const { chinaBandwidth, exChinaBandwidth, allBandwidth } = this.data.reduce(
      (pre: any, cur: any) => {
        return {
          chinaBandwidth:
            pre.chinaBandwidth > cur.areaCdnUsageMap.cn.totalBandwidth
              ? pre.chinaBandwidth
              : cur.areaCdnUsageMap.cn.totalBandwidth,
          exChinaBandwidth:
            pre.exChinaBandwidth > cur.areaCdnUsageMap.abroad.totalBandwidth
              ? pre.exChinaBandwidth
              : cur.areaCdnUsageMap.abroad.totalBandwidth,
          allBandwidth:
            pre.allBandwidth > cur.areaCdnUsageMap.all.totalBandwidth
              ? pre.allBandwidth
              : cur.areaCdnUsageMap.all.totalBandwidth,
        }
      },
      {
        chinaBandwidth: 0,
    ...
    let allBandwidth95 = 0,
      chinaBandwidth95 = 0,
      exChinaBandwidth95 = 0
    for (let i = this.data.length - 1; i >= 0; i--) {
      if (this.data[i].areaCdnUsageMap.all && this.data[i].areaCdnUsageMap.all.monthBandwidth95 > 0) {
        allBandwidth95 = this.data[i].areaCdnUsageMap.all.monthBandwidth95
        break
      }
    }
    for (let i = this.data.length - 1; i >= 0; i--) {
      if (this.data[i].areaCdnUsageMap.cn && this.data[i].areaCdnUsageMap.cn.monthBandwidth95 > 0) {
        chinaBandwidth95 = this.data[i].areaCdnUsageMap.cn.monthBandwidth95
        break
      }
    }
    for (let i = this.data.length - 1; i >= 0; i--) {
      if (this.data[i].areaCdnUsageMap.abroad && this.data[i].areaCdnUsageMap.abroad.monthBandwidth95 > 0) {
        exChinaBandwidth95 = this.data[i].areaCdnUsageMap.abroad.monthBandwidth95
        break
      }
    }
    const bandwidth95 = [
      {
        type: this.$t('All'),
        value: allBandwidth95,
      },
      {
        type: this.$t('China'),
        value: chinaBandwidth95,
      },
      {
        type: this.$t('Ex-China'),
        value: exChinaBandwidth95,
      },
    ]
    dataArray = [
      {
      ...
      },
      {
        index: '1',
        type: this.$t('China Bandwidth Peak'),
        dataType: 'fusionCDNChina',
        value: chinaBandwidth,
        restName: this.$t('Rest'),
        content: 'Mbps',
      },
      {
        index: '2',
        type: this.$t('Ex-China Bandwidth Peak'),
        dataType: 'fusionCDNExChina',
        value: exChinaBandwidth,
        restName: this.$t('Rest'),
        content: 'Mbps',
      },
      {
        index: '3',
        type: this.$t('95 Bandwidth Peak'),
        dataType: 'fusionCDN',
        value: bandwidth95,
        restName: this.$t('Rest'),
        content: 'Mbps',
  ...
  async changeBreadcrumb() {
    ...
    routeList.push(
      ...
      },
      {
        path: this.$route.fullPath,
        meta: {
          breadcrumb: 'FusionCDNBandwidthTitle',
        },
      }
    )
