  ...
  template: `
    <div v-loading="loading">
      <div class="text-center empty-content">
        ...
        <div v-if="isEmpty" clasetTypess="mx-auto heading-grey-13 mt-20">{{ $t('NoUsage') }}</div>
      </div>
...
export default class AllView extends Vue {
  loading = false
  isEmpty = false
  data: any = []
  tableData: any = []
  ...
  getLineChartInfo() {
    ...
    if (this.settings.includes('Separate SD')) {
      flatData = this.data.map((x: any) => [
        {
          ...
          usage: x.premiumAudioAudience + x.audioHost,
          type: this.$t('audio'),
        },
        { date: moment(x.date).format(displayFormat), usage: x.sdHost + x.premiumSdAudience, type: this.$t('sd') },
        { date: moment(x.date).format(displayFormat), usage: x.hdHost + x.premiumHdAudience, type: this.$t('hd') },
        {
          date: moment(x.date).format(displayFormat),
          usage: x.video1080pHost + x.premium1080pAudience,
          type: this.$t('Video Total Duration(Full HD)'),
        },
        {
          date: moment(x.date).format(displayFormat),
          usage: x.video2kHost + x.premium2kAudience,
          type: this.$t('Video2K'),
        },
        {
          date: moment(x.date).format(displayFormat),
          usage: x.video4kHost + x.premium4kAudience,
          type: this.$t('Video2K+'),
        },
      ])
    } else {
      flatData = this.data.map((x: any) => [
        ...
        {
          date: moment(x.date).format(displayFormat),
          usage: x.sdHost + x.premiumSdAudience + x.hdHost + x.premiumHdAudience,
          type: this.$t('hd'),
        },
        {
          date: moment(x.date).format(displayFormat),
  ...
  getBarChartInfo() {
    ...
    const { audioSum, sdSum, hdSum, hdpSum, video2kSum, video4kSum } = this.data.reduce(
      (pre: any, cur: any) => {
        return {
          audioSum: pre.audioSum + cur.audioHost + cur.premiumAudioAudience,
          sdSum: pre.sdSum + cur.sdHost + cur.premiumSdAudience,
          hdSum: pre.hdSum + cur.hdHost + cur.premiumHdAudience,
          hdpSum: pre.hdpSum + cur.video1080pHost + cur.premium1080pAudience,
          video2kSum: pre.video2kSum + cur.video2kHost + cur.premium2kAudience,
          video4kSum: pre.video4kSum + cur.video4kHost + cur.premium4kAudience,
        }
      },
    ...
    try {
      ...
      this.tableData = this.data.map((x: any) => {
        ...
        if (this.settings.includes('Separate SD')) {
          return {
            date: x.date,
            audio: x.audioHost + x.premiumAudioAudience,
            sd: x.sdHost + x.premiumSdAudience,
            hd: x.hdHost + x.premiumHdAudience,
            hdp: x.video1080pHost + x.premium1080pAudience,
            video2k: x.video2kHost + x.premium2kAudience,
            video4k: x.video4kHost + x.premium4kAudience,
            total: total,
          }
        } else {
          return {
            date: x.date,
            audio: x.audioHost + x.premiumAudioAudience,
            hd: x.sdHost + x.premiumSdAudience + x.hdHost + x.premiumHdAudience,
            hdp: x.video1080pHost + x.premium1080pAudience,
            video2k: x.video2kHost + x.premium2kAudience,
            video4k: x.video4kHost + x.premium4kAudience,
            total: total,
