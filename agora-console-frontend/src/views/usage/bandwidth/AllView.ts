  ...
  template: `
    <div v-loading="loading">
      ...
      >
        ...
        <el-table-column
          prop="totalMax"
          :label='this.$t("totalPeak")'
          label-class-name="table-title"
          class-name="table-content"
        >
          <template slot-scope="scope">
            {{ scope.row.totalMax | formatUsage('bandwidth') }}
          </template>
        </el-table-column>
      </el-table>
...
export default class AllView extends Vue {
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => {
      if (this.peakValue < x.totalMax) {
        this.peakValue = x.totalMax
      }
      return [
        { date: moment(x.date).format('MM-DD'), usage: x.totalMax, type: this.$t('totalPeak'), format: 'channel' },
      ]
    })
  ...
  async getUsageInfo() {
    this.loading = true
    try {
      if (this.condition.projectId === '0' && this.settings.includes('Bandwidth Aggregate')) {
        ...
        for (const data of usageData) {
          if (data.date in usageObj) {
            usageObj[data.date] = usageObj[data.date] + data.totalMax
          } else {
            usageObj[data.date] = data.totalMax
          }
        }
        const dataArray = []
        for (const date of Object.keys(usageObj)) {
          dataArray.push({ date, totalMax: usageObj[date] })
        }
        this.data = dataArray
      } else {
  ...
  init() {
    ...
    if (projectSetting) this.settings = projectSetting['Bandwidth'] || []
  }
