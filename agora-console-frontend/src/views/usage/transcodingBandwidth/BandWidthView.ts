    ...
    <div>
      <div class="usage-title-box-with-remaining d-flex justify-between">
        <div class="d-flex align-center">
          <div
            class="tab-button"
            v-if='settings.includes("Transcoding Bandwidth Audience")'
            :class='{ "active-btn": condition.type === "Transcoding Bandwidth Audience", "switch-btn": true }'
            @click='setType("Transcoding Bandwidth Audience")'
          >
            {{ $t('botPeak') }}
          </div>
...
export default class BandWidthView extends Vue {
  settings: any = ['All']
  condition: any = {
    type: 'Transcoding Bandwidth Audience',
    intervalType: 0,
    fromTs: undefined,
    endTs: undefined,
    model: 'bandwidth',
  ...
  async setType(type: string) {
    ...
    this.$router.push({
      name: 'usage.transcoding.bandwidth.' + this.condition.type,
  ...
  init() {
    ...
    if (projectSetting) {
      this.settings = projectSetting['Transcoding Bandwidth'] || []
    }
  ...
  async changeBreadcrumb() {
    ...
    routeList.push(
      ...
      },
      {
        path: this.$route.fullPath,
        meta: {
          breadcrumb: this.$t('Media Push') + ' - ' + this.$t('peakText'),
        },
      }
    )
