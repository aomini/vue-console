  ...
  template: `
    <div v-loading="loading">
      ...
      >
        ...
        <el-table-column prop="dau" :label='this.$t("dau")' label-class-name="table-title" class-name="table-content">
...
export default class DAUView extends Vue {
  ...
  getLineChartInfo() {
    ...
    const flatData = this.data.map((x: any) => {
      ...
      return [{ date: moment(x.date).format('MM-DD'), usage: x.dau, type: this.$t('dau'), format: 'channel' }]
    })
