  ...
  template: ` <div class="page-v3 moderation-page">
    <div class="card" v-loading="loading">
      <el-form :model="data" size="small" label-width="180px" ref="submit-form" :rules="rules">
        <div class="module-item-title">{{ $t('Client Screenshot Upload Configuration') }}</div>
        <div class="w-500 m-auto">
          <el-form-item :label="$t('Supervision callback url') + ':'" prop="callback_url_supervisor">
    ...
    <el-dialog :title='$t("Confirm")' :visible.sync="showConfirmDialog" width="380px">
      <div class="p-2">
        <div>
          <span>{{ $t('Confirm to update client screenshot uploading') }} </span>
        </div>
...
export default class EditModerationView extends Vue {
  ...
  data: any = {
    service_type: [ServiceType.Supervision],
  ...
  async updateConfig() {
    try {
      this.loading = true
      if (!this.data.service_type.find((item: ServiceType) => item === ServiceType.Supervision)) {
        this.data.service_type.push(ServiceType.Supervision)
      }
  ...
  async changeBreadcrumb() {
    ...
    routeList.push(
      ...
      },
      {
        ...
        meta: {
          breadcrumb: 'Client Screenshot Upload Configuration',
        },
      }
    )
